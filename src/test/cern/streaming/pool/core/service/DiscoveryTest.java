...
/**
 * Test for the discovery of Streams using {@link StreamFactory} and
 * {@link LazyPool}.
 * 
 * @author acalia
 */
public class DiscoveryTest {
	...
	public void testTwoFactoriesProvideTheSameId() {
		...
		StreamFactory factoryA = StreamFactoryMock.<String> newFactory().withIdProvideStreamWithValue(id, ANY_VALUE)
				.build();
		StreamFactory factoryB = StreamFactoryMock.<String> newFactory().withIdProvideStreamWithValue(id, ANY_VALUE)
				.build();
	...
	public void testDiscoveryXYZWithFactoryBuilder() {
		...
		StreamFactory factoryA = StreamFactoryMock.<String> newFactory().withIdDiscoverAnother(idY, idZ).build();

		StreamFactory factoryB = StreamFactoryMock.<String> newFactory().withIdDiscoverAnother(idX, idY)
	...
	public void testComplexFactory() {
		...
		StreamFactory factoryA = StreamFactoryMock.<String> newFactory().withIdDiscoverAnother(idX, idY1)
				.withIdDiscoverAnother(idX, idY2).build();

		StreamFactory factoryB = StreamFactoryMock.<String> newFactory().withIdProvideStreamWithValue(idY1, ANY_VALUE)
				.withIdDiscoverAnother(idY2, idZ).build();

		StreamFactory factoryC = StreamFactoryMock.<String> newFactory().withIdProvideStreamWithValue(idZ, ANY_VALUE)
				.build();
