...
public interface TypedStreamFactory <X, T extends StreamId<X>> extends StreamFactory {
    ...
    default <Y> Optional<ReactiveStream<Y>> create(StreamId<Y> id, DiscoveryService discoveryService) {
        if(!streamIdClass().isAssignableFrom(id.getClass())) return Optional.empty();
    ...
    /**
     * Given a generic {@link StreamId} this method returns a {@link Boolean} indicating whether the {@link TypedStreamFactory}
